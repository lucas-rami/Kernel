...
int register_handler(uintptr_t handler_function, uint8_t gate_type,
                     ...
                     uint16_t segment) {
  // Sanity precheck
  if (segment != SEGSEL_KERNEL_CS) {
    lprintf("Segment is not SEGSEL_KERNEL_CS\n");
    return -1;
  }

  // Extract the IDT base and create the IDT entry
  uint32_t *idt = (uint32_t *)idt_base();
  uint32_t offset_lower = handler_function & TWO_LSB_MASK;
  uint32_t offset_upper =
      (handler_function & TWO_MSB_MASK) >> BITS_IN_TWO_BYTES;
  uint32_t interrupt_gate_descriptor_upper =
      ((offset_upper << BITS_IN_TWO_BYTES) | WORKING_GATE | SIZE_GATE_32 |
       ((int)privilege_level << 13));
